var inka3d =
{
	...
	// createRenderer() returns a renderer object with these methods:
	...
	createRenderer: function(numRenderJobs)
	{
		...
		var renderer = {
			...
			pickPixel: new Uint8Array(4), nextId: 1, idPool: []
		};
	...
	// dataURL: url to data file (e.g. "foo.dat")
	// texURL: url to directory containing texture image files (e.g. 'textures/')
	// ext: extension of image files (e.g. '.jpg')
	// onload(container): get called if container was loaded. container has these methods:
	...
	//   - onprogress(progress): gets called for every resource that was loaded. goes from 0 to 1
	//   - progress: current progress from 0 to 1
	loadEmbedded: function(container, dataURL, texURL, ext, onload)
	{
		var loader =
		{
			onload: onload,
		...
		inka3d.loadInternal(container, dataURL, texURL, ext, loader);
		return loader;
	},

	// load scene container from json file. returns a loader object
	// jsonURL: url to json scene container file
	// dataURL: url to data file (e.g. "foo.dat")
	// texURL: url to directory containing texture image files (e.g. 'textures/')
	// ext: extension of image files (e.g. '.jpg')
	// onload(container): get called if container was loaded. container has these methods:
	...
	//   - onprogress(progress): gets called for every resource that was loaded. goes from 0 to 1
	//   - progress: current progress from 0 to 1
	loadJSON: function(json, dataURL, texURL, ext, onload)
	{
		var loader =
		{
			onload: onload,
		...
		xhr.onload = function (event)
		{
			if (this.status != 200 && this.status != 0)
				alert('Could not load json file ' + json);
...
// internal helper functions
	...
	loadInternal: function(container, dataURL, texURL, ext, loader)
	{
		container.instances = [];
		...
		container.createScene = function(name, group)
		{
			...
			{
				return inka3d.getIntVector(name, length, scene.attributes, instance.istate);
			};
			instance.getIntArray = function(name)
			{
				return inka3d.getIntArray(name, scene.attributes, instance.istate);
			};
			instance.getFloatVector = function(name, length)
			{
				return inka3d.getFloatVector(name, length, scene.attributes, instance.fstate);
			};
			instance.getFloatArray = function(name)
			{
				return inka3d.getFloatArray(name, scene.attributes, instance.fstate);
			};
			instance.getTexture = function(name, index)
			{
				return inka3d.getTexture(name, index, scene.attributes, instance.ostate);
			};
			instance.setTexture = function(name, texture, index)
			{
				inka3d.setTexture(name, texture, index, scene.attributes, instance.ostate);
			};
			instance.getObjectId = function(name)
			{
				return inka3d.getObjectId(name, scene.objects, instance.ids, group.renderer);
			};			
		...
		var textures = container.textures;
		for (var name in textures)
			textures[name].initGlobal(textures[name], texURL, ext, loaded);
		...
		{
			if (this.status != 200 && this.status != 0)
				alert('Could not load data file ' + dataURL);
	...
	getObjectId: function(name, infos, ids, renderer)
	{
		...
		if (ids[index] == 0)
		{
			if (renderer.idPool.length > 0)
				ids[index] = renderer.idPool.pop();
			else
				ids[index] = renderer.nextId++;
		}
		return ids[index];
	}
};
